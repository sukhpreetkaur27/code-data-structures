...
In other words, they point to two different locations in memory, while the nodes with value 8 in A and B (3rd node in A and 4th node in B) point to 
 ...
 *
 */
public class _20_IntersectionLL {
   ...
   * @return
   */
  public _ListNode intersection(_ListNode headA, _ListNode headB) {
    /**
     * calculate length of A
     */
    int lenA = 0;
    _ListNode curr = headA;
    while (curr != null) {
      curr = curr.next;
      lenA++;
    }
    ...
    if (lenA > lenB) {
      return intersection(headA, headB, lenA - lenB);
    } else {
      return intersection(headB, headA, lenB - lenA);
    }
  }

  private _ListNode intersection(_ListNode headA, _ListNode headB, int diff) {
    while (diff > 0) {
      headA = headA.next;
      diff--;
    }
