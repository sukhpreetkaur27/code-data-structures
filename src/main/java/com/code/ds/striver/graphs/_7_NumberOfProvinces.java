 ...
 *
 */
public class _7_NumberOfProvinces {
  ...
  public int findCircleNum(int[][] isConnected) {
    ...
    for (int i = 0; i < n; i++) {
      /**
       * Start from each node to its connections
       */
      if (!visited[i]) {
        provinces++;
  ...
  private void dfs(int node, boolean[] visited, int[][] isConnected) {
    visited[node] = true;
    for (int v = 0; v < isConnected[node].length; v++) {
      /**
       * if connectivity exists and the node isn't visited --> traverse it
       */
