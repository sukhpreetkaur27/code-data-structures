 ...
 *  when the array is arranged in ascending order. If two or more are same in the array then their rank is also the same as the rank of the first occurrence of the element. 
...
increases by 1.
Your Task:
Complete the function int replaceWithRank(), which takes integer N  and an array of N integers as input
 and returns the list in which element at each position of original array is replaced by the rank of that element.
...
public class _48_Rank {
  ...
  public int[] replaceWithRank(int arr[], int n) {
    ...
    Arrays.sort(copy);
    for (int i = 0; i < n; i++) {
      rank.putIfAbsent(copy[i], rank.size() + 1);
    }
    for (int i = 0; i < n; i++) {
